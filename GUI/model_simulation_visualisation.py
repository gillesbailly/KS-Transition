...
class Model_Simulation_Visualisation( Serie2DGallery ):
    ...
    ############################
    def update_figure( self, model_simulation_vec, user_data_vec ):
        bar_width = 0.2
        opacity   = 0.8
        self.figure.clear()
        n_rows = 4
        n_cols = 1
        df = user_data_vec_to_data_frame( user_data_vec )
 
        df       = user_data_vec_to_data_frame( user_data_vec )
        model_df = simulation_vec_to_data_frame( model_simulation_vec, user_data_vec )
        model_df = pd.concat( [df, model_df] )
        ...
        for i, model_name in enumerate( model_vec ) :
            ...
            sns.lineplot( x='block_id', y="time", hue="technique_name", style = 'model', style_order = style_order, data=df )
            plt.title( model_name )
            plt.ylabel( 'Time' )
            plt.xlabel( '' )
            plt.ylim( 0, 6 )
            if i == n_cols -1 :
                #ax.legend().title[0].set_text('Technique')
                #ax.legend().tilte[1].set_text('Data')
            ...
            sns.lineplot( x='block_id', y='success_plot', hue="technique_name", style = 'model', style_order = style_order, data=df )
            ...
            sns.lineplot( x='block_id', y='hotkey', hue="technique_name", style = 'model', style_order = style_order, data=df )
            plt.xlabel( '' )
            ...
            sns.lineplot( x='block_id', y='hotkey_success', hue="technique_name", style = 'model', style_order = style_order, data=df )
            plt.xlabel( 'Block id' )
